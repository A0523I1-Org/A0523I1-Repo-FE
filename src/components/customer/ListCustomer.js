...
const ListCustomer = () => {
    ...
    const handlePageClick = (event) => {
        ...
        setCurrentPage(event.selected+1);// Lưu lại giá trị của currentPage khi click vào nút phân trang
...
>>>>>>> 19ef6b4 (TrieuTLQ)
    ...
    const handleDeleteSelect = async () => {
        if (selectedIds < 1) {
            alert("Hãy chọn khách hàng muốn xóa")
        } else if (window.confirm("Bạn có muốn xóa những khách hàng đã chọn?")) {
            try {
                    ...
                    <tr key={customer.id} className="hover:bg-gray-50 bg-white md:border-none block md:table-row">
                        ...
                        <td className="px-6 py-4 md:border md:border-grey-500 text-left block md:table-cell">
                            <div className="flex justify-end gap-4">
                                <button x-data="{ tooltip: 'Delete' }" href="#" type="submit"
...
=======
        ...
        {showPopup && (
                <ConfirmationPopup
                    message={
                        popupAction === "single"
                            ? "Bạn có muốn xóa những khách hàng đó?"
